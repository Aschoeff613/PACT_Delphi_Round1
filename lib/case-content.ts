...
const CASE_CONTENT: Record<string, CaseContent> = {
  "all-tasks:0": {
    task_code: "T1",
    guiding_question: "How sick is this person, judged at a glance?",
    cognitive_demand: "",
    cluster: "Partly inside Stanford V4 D1 (high-risk rule-out); no standalone equivalent",
    task_definition: "A fast, holistic judgement of how unwell someone is, or how likely they are to deteriorate, formed at a glance rather than reasoned out. This first impression sets the pace of everything that follows.",
    example_ed: "A 78-year-old arrives by ambulance for generalized weakness with a heart rate of 96 and a normal blood pressure. From the doorway she is grey, quiet and not tracking. Judge how sick she is, and how quickly this needs to move, before any results are back.",
    example_primary_care: "A same-day walk-in with two days of vomiting has normal recorded vital signs but looks exhausted and cannot sit up on the exam table. Judge how unwell she is right now, and whether that changes the pace of the visit.",
  ...
  "all-tasks:1": {
    ...
    example_primary_care: "The appointment schedule is running 40 minutes behind with a double-booked slot, a same-day add-on for chest tightness, and two urgent portal messages. Decide what gets attention in the next hour and what is deferred.",
  ...
  "all-tasks:2": {
    ...
    construct_boundary: "Not whether the information can be trusted (task 7), and not integrating findings already accepted as accurate (task 8). Stopping the search is this task; an unresolvable unknown is task 5.",
    case_format: "Interactive or agentic: the physician must be able to request items one at a time",
    status: "V5 core",
  },
  "all-tasks:3": {
    task_code: "T4",
    guiding_question: "What explains these findings, and which is most likely?",
    cognitive_demand: "",
  ...
  "all-tasks:4": {
    ...
    task_definition: "Explicitly acknowledging what is unknown and choosing a next step that either tolerates it or resolves it, instead of forcing an answer too early. Includes safety-netting: naming in advance what would prompt a change of course.",
    example_ed: "A 30-year-old with 12 hours of periumbilical pain has an equivocal ultrasound and a normal white count. Appendicitis cannot be excluded tonight. Say so explicitly, then set the return threshold and recheck interval that make discharge acceptable.",
    example_primary_care: "An isolated mildly elevated alkaline phosphatase in an asymptomatic patient. Acknowledge that the cause cannot be established yet, deliberately leave it alone, and name the repeat interval and the value that would prompt a workup.",
  ...
  "all-tasks:7": {
    task_code: "T8",
    guiding_question: "What do these findings mean when taken together?",
    cognitive_demand: "",
    cluster: "Stanford V4 D4 contains integration inside the merged bias-resistance definition",
    task_definition: "Relating several findings, all accepted as accurate, to one another, to this patient's own baseline, and to how they were previously, in order to arrive at a single interpretation.",
    example_ed: "An 85-year-old's blood pressure is 104/60 — normal by population standards, but 40 points below his own documented baseline — and his creatinine has risen since a value six months ago. Interpret these findings together, and against his own baseline.",
    example_primary_care: "The patient feels well, her A1c is 11.2, her home glucose log shows values in the 120s, and last year's A1c was 6.8. All three are accepted as accurate. Give a single coherent interpretation.",
    construct_boundary: "Not judging whether a source is trustworthy (task 7), and not ranking candidate diagnoses (task 4). The findings are already accepted as accurate.",
  ...
  "all-tasks:8": {
    ...
    task_definition: "Retrieving stored medical knowledge, rules or standards from memory, or looking them up, and applying them to the case — including recognising the limits of one’s own knowledge.",
    example_ed: "A patient on apixaban has an intracranial bleed. Retrieve the reversal agent, the dose and the time window, and identify the point at which recall runs out and an outside source is needed.",
  ...
  "all-tasks:9": {
    ...
    task_definition: "Looking ahead to the likely trajectory, endpoint and next moves, and letting that forecast change what is done now, before a decision is reached.",
    example_ed: "A probable small bowel obstruction, not yet confirmed. Plan forward: if the CT confirms it, surgery is called and a nasogastric tube goes in now; if it is negative, the patient goes home. Sequence the immediate work so that it holds up either way.",
    example_primary_care: "A patient with early dementia is still driving and living alone. Both are tolerable today but likely to become unsafe as the dementia progresses. Project the coming year, then decide what to begin now rather than after a crash or a fall forces it — assessing decision-making capacity, addressing driving, identifying a surrogate decision-maker — and what to hold over to a follow-up visit.",
  ...
  "all-tasks:10": {
    ...
    task_definition: "Bringing everything together into a final disposition and plan, and judging whether a pending test or action is worth pursuing based on whether its result would actually change that plan.",
  ...
  "all-tasks:13": {
    ...
    example_primary_care: "At the end of a long appointment, notice your own engagement dropping and that you are rushing a complex patient. Slow down deliberately and re-check the medication list you have just reviewed.",
