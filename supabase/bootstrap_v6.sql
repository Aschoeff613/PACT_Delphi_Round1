...
--
--   Then seeds 3 sections and the 17 Erasmus V6 cognitive constructs
--   (6 Management / 8 Diagnostic / 3 Communication) with their paired
--   Emergency Department and Primary Care case seeds.
--
...
CREATE TABLE IF NOT EXISTS public.sections (
    ...
    name text NOT NULL,
    description text,
    CONSTRAINT sections_name_check CHECK ((name = ANY (ARRAY['Management'::text, 'Communication'::text, 'Diagnostic Reasoning'::text])))
);
...
SELECT v.slug, v.name, v.description
  FROM (VALUES
  ('management', 'Management', 'Review management-focused scenarios involving treatment choices, escalation, and workflow implications.'),
  ('diagnostic', 'Diagnostic Reasoning', 'Assess tasks centered on diagnostic reasoning, interpretation, and uncertainty.'),
  ('communication', 'Communication', 'Rate communication-heavy tasks, handoffs, patient interactions, and coordination.')
...
-- ────────────────────────────────────────────────────────────────────────────
-- Verification — expect: management 6, diagnostic 8, communication 3
-- ────────────────────────────────────────────────────────────────────────────
...
SELECT s.slug,
 ...
 GROUP BY s.slug
 ORDER BY CASE s.slug WHEN 'management' THEN 1 WHEN 'diagnostic' THEN 2 ELSE 3 END;
